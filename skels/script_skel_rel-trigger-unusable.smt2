...
(assert (= BAV1 (forall ((x Int)) (=> (>= x GEN1) (and (dummy x) (distinct (- (two_to_the k) GEN2) (* GEN3 x)))))))
(assert (= BAV2 (forall ((x Int)) (>= x GEN1))))
(assert (= BAV3 (forall ((x Int)) (and (dummy x) (distinct (- (two_to_the k) GEN2) (* GEN3 x))))))
(assert (= BAV4 (forall ((x Int)) (distinct (- (two_to_the k) GEN2) (* GEN3 x)))))
(check-sat)
(get-model)