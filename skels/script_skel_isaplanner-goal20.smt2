...
(assert (= BAV1 (forall ((x Nat)) (>= (nat-to-int x) GEN1))))
(assert (= BAV2 (forall ((x Nat)(y Nat)) (=> (= (nat-to-int x) (nat-to-int y)) (= x y)))))
(assert (= BAV3 (forall ((x Nat)(y Nat)) (= (nat-to-int x) (nat-to-int y)))))
(assert (= BAV4 (forall ((x Nat)(y Nat)) (= x y))))
(assert (= BAV5 (= (nat-to-int zero) GEN2)))
(assert (= BAV6 (forall ((x Nat)) (= (nat-to-int (succ x)) (+ GEN3 (nat-to-int x))))))
(assert (= BAV7 (forall ((x Nat)) (= (less x zero) false))))
(assert (= BAV8 (forall ((x Nat)) (= (less zero (succ x)) true))))
(assert (= BAV9 (forall ((x Nat)(y Nat)) (= (less (succ x) (succ y)) (less x y)))))
(assert (= BAV10 (forall ((x Nat)(y Nat)) (= (less x y) (< (nat-to-int x) (nat-to-int y))))))
(assert (= BAV11 (forall ((x Nat)(y Nat)) (< (nat-to-int x) (nat-to-int y)))))
(assert (= BAV12 (= (len nil) zero)))
(assert (= BAV13 (forall ((x Nat)(y Lst)) (= (len (cons x y)) (succ (len y))))))
(assert (= BAV14 (forall ((i Nat)) (= (insort i nil) (cons i nil)))))
(assert (= BAV15 (forall ((i Nat)(x Nat)(y Lst)) (= (insort i (cons x y)) (ite (less i x) (cons i (cons x y)) (cons x (insort i y)))))))
(assert (= BAV16 (= (sort nil) nil)))
(assert (= BAV17 (forall ((x Nat)(y Lst)) (= (sort (cons x y)) (insort x (sort y))))))
(assert (= BAV18 (forall ((l Lst)) (= (len (sort l)) (len l)))))
(check-sat)
(get-model)
(exit)