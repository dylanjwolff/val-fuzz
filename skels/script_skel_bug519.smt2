...
(assert (= BAV1 (>= (start GEN29) GEN30)))
(assert (= BAV2 (>= (start GEN31) GEN32)))
(assert (= BAV3 (>= (start GEN33) GEN34)))
(assert (= BAV4 (>= (start GEN35) GEN36)))
(assert (= BAV5 (>= (start GEN37) GEN38)))
(assert (= BAV6 (>= (start GEN39) GEN40)))
(assert (= BAV7 (= cost2 cost)))
(assert (= BAV8 (forall ((t Int)) (=> (<= GEN41 t cost2) (>= max-parallel (parallel t))))))
(assert (= BAV9 (forall ((t Int)) (<= GEN41 t cost2))))
(assert (= BAV10 (forall ((t Int)) (>= max-parallel (parallel t)))))
(assert (= BAV11 (< cost GEN42)))
(check-sat)
(get-model)
(assert (< cost GEN42))
(check-sat)